...
// Forge-style settings based on the supplied MediaInfo.
// Thread count is intentionally limited for Railway memory/CPU stability.
const X264_PARAMS = [
  "ref=1",
  "bframes=2",
  "b-pyramid=0",
...
app.post("/api/encode", upload.single("video"), async (req, res) => {
  if (!req.file) {
    return res.status(400).json({
      ok: false,
      error: 'Missing video file. Use multipart/form-data field "video".'
    });
  }
  ...
  try {
    ...
    console.error("[ENCODE ERROR]", detail);
